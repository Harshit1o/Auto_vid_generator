    ...
    for attempt in range(retry_count):
        try:
            ...
            if attempt < retry_count - 1:
                print(f"Attempt {attempt+1} failed for {selector}: {str(e)}. Retrying...")
...
async def run():
    ...
    async with async_playwright() as p:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if attempt < max_attempts - 1:
                    print(f"Attempt {attempt+1} failed to find Voice Only option: {str(e)}. Retrying...")
        ...
        if not await wait_and_click(page, "text='Continue'"):
            print("Trying alternative method for first Continue button")
        ...
        if not await wait_and_click(page, "text='Continue'", timeout=20000):
            print("Trying alternative method for second Continue button")
            continue_buttons = await page.query_selector_all("button")
            clicked = False
            for button in continue_buttons:
                button_text = await button.text_content()
                if "Continue" in button_text:
                    ...
                    break
            if not clicked:
                print("Could not find second Continue button, trying to proceed anyway")
        ...
        try:
            ...
            print("ℹTerms popup didn't appear, continuing...")
        ...
        try:
            ...
            print(f"Sign-in process issue: {str(e)}")
